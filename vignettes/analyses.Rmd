...
Traditional Gleason grade is the sum of Gleason major and Gleason minor. This can be exemplified as being clearly associated with biochemical recurrence (BCR) via log-rank tests in Kaplan-Meier curves:
...
```{r tcga_decipher_bcr}
matched_tcga <- MatchedAssayExperiment(mae_tcga)
clin_tcga_subset <- clin_tcga[row.names(colData(matched_tcga)), ]
...
tcga_decipher <- mae_tcga[["scores"]]["decipher", row.names(colData(matched_tcga))]
coxph_tcga_decipher <- coxph(bcr_tcga_subset ~ tcga_decipher)
summary(coxph_tcga_decipher)
```

```{r taylor_decipher_bcr}
matched_taylor = MatchedAssayExperiment(mae_taylor)
clin_taylor_subset <- clin_taylor[row.names(colData(matched_taylor)), ]
...
taylor_decipher <- mae_taylor[["scores"]]["decipher", row.names(colData(matched_taylor))]
