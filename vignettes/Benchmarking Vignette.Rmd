...
-   TCGA in package for scores has 417 samples but there are 461 gene expression samples
...
# Decipher
decipher_genes <- c(getDecipherOverGenes(),
                    getDecipherUnderGenes())
decipher_genes <- lapply(decipher_genes, FUN=expandAliases)
...
```{r, echo = F}
TCGA_gex = prepGEX(GEX = mae_tcga@ExperimentList$gex.rsem.log, gene_list = oncotype_genes, log_transform = TRUE)
ground_truth = oncotype_score(TCGA_gex)
...
correlations = lapply(setNames(gene_range, gene_range), function(genes_to_remove){
  ...
  sapply(combos, function(genes_removed){
    cor(ground_truth, TCGA_gex %>% select(-!!genes_removed) %>% oncotype_score)
  })
})
...
```{r}
KUND_gex = prepGEX(mae_kunderfranco@ExperimentList$gex.logr, 
                   lapply(getProlarisGenes(), expandAliases), 
                   log_transform = FALSE)
ground_truth = prolaris_score(KUND_gex)
```
...
correlations = lapply(setNames(gene_range, gene_range), function(genes_to_remove){
  ...
  sapply(combos, function(genes_removed){    
    cor(ground_truth, KUND_gex %>% select(-!!genes_removed) %>% prolaris_score())
  })
})
...
```{r}
decipher_genes_over <- lapply(getDecipherOverGenes(), expandAliases)
decipher_genes_under <- lapply(getDecipherUnderGenes(), expandAliases)
```
...
```{r}
TCGA_gex = prepGEX(mae_tcga@ExperimentList$gex.rsem.log,
...
```{r}
ground_truth = decipher_score(TCGA_gex)
```
...
correlations = lapply(setNames(gene_range, gene_range)[1:6], function(genes_to_remove){
  ...
  sapply(combos, function(genes_removed){
    cor(ground_truth, decipher_score(TCGA_gex %>% select(-genes_removed)))
  })
})
...
TCGA_gex = prepGEX(mae_tcga@ExperimentList$gex.rsem.log,
                   ar_genes,
                   log_transform = F)
ground_truth = ar_score(TCGA_gex)
```
...
correlations = lapply(setNames(gene_range, gene_range), function(genes_to_remove){
  ...
  #apply the combination of genes to the TCGA prepped data to calculate the affected risk score
  sapply(combos, function(genes_removed){
    cor(ground_truth, ar_score(TCGA_gex %>% select(-genes_removed)))
  })
})
