...
1. `gex_*` slots contain gene expression values, with the suffix wildcard indicating unit or method for gene expression
2. `cna_*` slots contain copy number values, with the suffix wildcard indicating method for copy number alterations
3. `mut` slots contain somatic mutation calls
4. `MultiAssayExperiment::colData(maeobj)` contains the clinical metadata based on a pre-defined template
