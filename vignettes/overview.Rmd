...
#template <- utils::read.csv("data-raw/template_prad.csv")
...
for(mae in studies[,"MAE-object"]){
	...
	if(mae == "mae_abida"){
		...
		studies[w,"Data source"] <- "Genomics Data Commons (GDC)"
...
Studies that have been generated using the same array platform, such as HG-U133A (GEO platform code GPL96) for Sun et al. and Wang et al. have same gene annotations, as these platforms have been processed using the same latest annotation information. Similarly, HuEx-1_0-st in Taylor et al. (GPL5188) has the same gene annotations as Kim et al.

Unsupervised learning is a useful technique for generating a ballpark view to the datasets. For example, one can quickly inspect whether the expression values overlap greatly, or if there are consistent differences, such as batch effects between studies. Technical variation in data processing between studies has been minimized by using a harmonized processing pipeline, in this case by processing the raw CEL files using RMA in similar manner for all studies. 

Let us plot Principal Component Analysis (PCA) plots for these two studies with overlapping genes, and plot the two orthogonal axes with highest variation by projecting down the whole transcriptome to just these two principal components (PC):
...
Three primary clinical end points were utilized:
- Gleason grade
- Recurrence
- Overall survival (OS)
...
knitr::kable(gleasons, caption="Gleason grades in curatedPCaData")
```

```{r recurrences, results = 'asis', echo=FALSE}
knitr::kable(recurrences, caption="Disease recurrence end point in curatedPCaData")
```

```{r os, results = 'asis', echo=FALSE}
knitr::kable(survivals, caption="Overall survival end point in curatedPCaData")
```
...
```{r}
head(mae_taylor[["cibersort"]])[1:5,1:5]
```

Similarly to access results from the other methods, the following can be run
```{r}
head(mae_taylor[["quantiseq"]])[1:5,1:5]
head(mae_taylor[["xcell"]])[1:5,1:5]
head(mae_taylor[["epic"]])[1:5,1:5]
head(mae_taylor[["mcp"]])[1:5,1:5]
```
...
Tumor sample purities were run for studies which had separate normal samples (and their gene expression) available.

The purity scores were calculated using DeMixT [Wang et al.], and was available for the studies:
- ```mae_chandran```
- ```mae_friedrich```
- ```mae_kunderfranco```
- ```mae_taylor```
...
The purity scores are stored in slot ```purity``` and can be accessed via:

```{r purities}
mae_tcga[["purity"]][,1:10]
```

The metadata slot provides the original purity values as estimated by the original authors, if they provided these pre-calculated values. For example in TCGA, our estimated DeMixT values vs. the original values:

```{r demixt_own_vs_authors, dpi=150, fig.width=7, fig.height=7, out.width="100%", fig.cap="DeMixT from curatedPCaData vs. directly from TCGA"}
tcga_demixts <- wideFormat(mae_tcga[c("demixt"),,"purity"], colDataCols="tumor_purity_demixt")
plot(as.matrix(tcga_demixts[,2:3]), pch=16, xlab="DeMixT from curatedPCaData's GDC processed FPKM-UQ", ylab="Original DeMixT from TCGA")
```
...
- ```Decipher``` [Herlemann et al., 2020]
- ```Oncotype DX``` [Knezevic et al., 2013]
- ```Prolaris``` [NICE Advice, 218]

Further, the 20-gene Androgen Receptor (AR) score is calculated as described in the TCGA's Cell 2015 paper and provided in the ```scores``` slot on the row:

- ```AR_score``` [Cancer Genome Atlas Research Network, 2015]
